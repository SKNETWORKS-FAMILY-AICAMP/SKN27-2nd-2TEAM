...
- **단순 파생 변수와 피처 선정의 한계**: 제공된 기본 피처와 단순 집계 수준의 파생 변수만으로는 이탈 여부에 영향을 미치는 결정적 패턴을 도출하기 어려웠으며, 다수 변수를 복잡하게 엮어내는(Feature Crossing) 심층 분석 역량 부족으로 피처 선정에 난항을 겪음.

- **초기 성능 저하 및 모델링 시도 부족**: 피처 발굴의 한계가 곧 베이스라인 모델의 성능 부진으로 이어졌고, 이를 보완하느라 정작 하이퍼파라미터 튜닝이나 앙상블 등 다양하고 발전된 알고리즘을 깊게 테스트해 볼 시간적/기술적 여유가 부족했음.

### 향후 개선 방향

- **데이터 분석 순서 정립**: '원본 EDA → 결측 패턴 분석 → 통계적 근거 기반 전처리' 순으로 프로세스를 재설계하여 데이터 왜곡 최소화.
  
- **불균형 데이터 대응**: SMOTE 또는 ADASYN과 같은 오버샘플링 기법을 적용하여 이탈군 데이터를 보강하고, 정확도(Accuracy) 대신 F1-Score  핵심 지표로 설정.
